...
var onHttpTrigger = async (runtime2, payload) => {
  runtime2.log(`HTTP trigger received for document notarization`);
  try {
    const {
      documentHash,
      documentType,
    ...
    const finalDocumentId = documentId || keccak256(encodePacked(["string", "string", "address", "uint256"], [documentHash, documentType, notarizerAddress, BigInt(Date.now())]));
    runtime2.log(`Processing document: ${finalDocumentId}`);
    const evmClient = runtime2.getCapability(ClientCapability, {
      chainSelector: runtime2.config.chainSelector
    });
    const documentTypeEnum = documentType === "FIXED" ? 0 : 1;
    const reportData = encodeAbiParameters(parseAbiParameters("bytes32, bytes32, uint8, address"), [
      finalDocumentId,
      documentHash,
      documentTypeEnum,
      notarizerAddress
    ...
    const signedReport = runtime2.report(Buffer.from(reportData.slice(2), "hex").toString("base64"));
    runtime2.log(`Generated signed report`);
    await evmClient.writeReport({
      contractAddress: runtime2.config.contractAddress,
      signedReport
    }).result();
    runtime2.log(`Document ${finalDocumentId} successfully notarized on chain`);
    return {
      success: true,
    ...
    runtime2.log(`Error processing document notarization: ${error}`);
    return {
      success: false,
